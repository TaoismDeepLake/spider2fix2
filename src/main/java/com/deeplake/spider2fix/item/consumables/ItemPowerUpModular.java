package com.deeplake.spider2fix.item.consumables;

import com.deeplake.spider2fix.entity.creatures.EntityModUnit;
import com.deeplake.spider2fix.item.ItemBase;
import com.deeplake.spider2fix.util.CommonFunctions;
import com.deeplake.spider2fix.util.EntityUtil;
import com.deeplake.spider2fix.util.NBTStrDef.IDLNBTUtil;
...
import static com.deeplake.spider2fix.util.NBTStrDef.IDLNBTDef.MARK_TOTAL_COUNT;
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer playerIn, EntityLivingBase target, EnumHand handIn) {
        if (target instanceof EntityModUnit)
        {
            ...
            if (legalTarget.is_mechanic)
            {
                ...
                {
                    CommonFunctions.SafeSendMsgToPlayer(playerIn, "spider2fix.msg.upgrade_module.req_lv", String.valueOf(requireLv));
                    return false;
                }
                ...
                if (!playerIn.world.isRemote)
                {
                    ...
                    if (success)
                    {
                        ...
                        CommonFunctions.SafeSendMsgToPlayer(playerIn, "spider2fix.msg.upgrade_module.ok", valueBefore, valueAfter);
