package com.deeplake.spider2fix.item.skills;

import com.deeplake.spider2fix.IdlFramework;
import com.deeplake.spider2fix.init.ModCreativeTab;
import com.deeplake.spider2fix.item.ItemBase;
import com.deeplake.spider2fix.util.CommonFunctions;
import com.deeplake.spider2fix.util.IDLSkillNBT;
import com.deeplake.spider2fix.util.MessageDef;
...
import static com.deeplake.spider2fix.util.CommonDef.G_SKY;
import static com.deeplake.spider2fix.util.CommonDef.TICK_PER_SECOND;
import static com.deeplake.spider2fix.util.CommonFunctions.SendMsgToPlayer;
import static com.deeplake.spider2fix.util.CommonFunctions.isShiftPressed;
import static com.deeplake.spider2fix.util.MessageDef.getSkillCastKey;
...
public class ItemSkillBase extends ItemBase {
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "spider2fix.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("spider2fix.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("spider2fix.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("spider2fix.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("spider2fix.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("spider2fix.skill.shared.lv_max") : "";

        return I18n.format("spider2fix.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("spider2fix.skill.shared.duration_desc", val);
    }
